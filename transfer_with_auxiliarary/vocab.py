...
preprocess = TextPreprocessing(
        twitter=False, replace_number=True, clean_html=True,
...
def from_article_db(path):
    dataset = pd.read_csv(open(path,'rb'))
    captions = []
    for i, row in enumerate(dataset.iterrows()):
        captions.append(row[3])
    return captions
...
def build_vocab(data_path, data_name, jsons, threshold):
    ...
    for path in jsons[data_name]:
        ...
        elif data_name == 'artible_db':
            captions = from_article_db_csv(full_path)
        else:
            captions = from_txt(full_path)
        for i, caption in enumerate(captions):
            #tokens = nltk.tokenize.word_tokenize(
            #    caption.lower().decode('utf-8'))
            tokens = preprocess(str(caption))
