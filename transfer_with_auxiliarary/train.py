...
def main():
    ...
    parser.add_argument('--val_caption_path', default='.',type=str,
    ...
    parser.add_argument('--unfreeze_word_emb', default=100,type=int)
    parser.add_argument('--unfreeze_all', default=100,type=int)
    parser.add_argument('--books_path', default='./',type=str)
    ...
    # merge with FastTextVocab
    #path_to_binary_file='/mnt/storage01/fangyu/fasttext_embeddings/cc.'+lang+'.300.bin'
    #path_to_binary_file='/home/fangyu/data/MUSE/cc.de.300.vec'
    ...
    ft_vocab = FastTextVocab(
                      include_word=True,
                      min_subword=4, max_subword=6,
                      word_min_freq=1, subword_min_freq=1)
    ft_vocab.create_index_mapping(
            embeddings, vocab=vocab.word2idx)
    
    # save vocab
    with open('./vocab/ft_vocab_'+lang+'_min4_max_6_1_3_.pkl','wb') as f:
        pickle.dump(ft_vocab,f,pickle.HIGHEST_PROTOCOL)
        print ('[ft vocab saved.]')
    """
    ...
    _ = None
    if opt.only_test:
        train_loader, val_loader,_, (val_loader_cap,val_loader_art,val_loader_title,val_loader_lead) = data.get_loaders(opt.data_name, ft_vocab, opt.crop_size, opt.batch_size, opt.workers, opt)
        val_loaders = [val_loader,val_loader_cap,val_loader_art,val_loader_title,val_loader_lead] 
    elif opt.train_de:
        train_loaders, val_loader, _ = data.get_loaders(
            opt.data_name, ft_vocab, opt.crop_size, opt.batch_size, opt.workers, opt)
        val_loaders = [val_loader]
    else:
        train_loaders = []
        books = os.listdir(opt.books_path)
        books = sorted(books)
        #print (books)
        for name in books:
            print (name)
            opt.caption_path = os.path.join(opt.books_path, name)
    ...
    if opt.resume:
        if os.path.isfile(opt.resume):
            ...
            m = load_matrix('subword.fr-de.vec-mat.prev.sota')
    ...
    #rsum = validate(opt, val_loaders[0], model)
    #print ('******* caption val *******')
    #rsum_cap = validate(opt, val_loaders[1], model)
    #print ('******* article val *******')
    #rsum_art = validate(opt, val_loaders[2], model)
    #print ('******* title val *******')
    #rsum_title = validate(opt, val_loaders[3], model)
    #print ('******* lead val *******')
    #rsum_lead = validate(opt, val_loaders[4], model)
    #'''
    ...
    for epoch in range(opt.num_epochs):
        ...
        # train for one epoch
        if opt.train_de:
            train(opt, train_loaders, model, epoch, val_loaders)
        else:
            train(opt, train_loaders[epoch], model, epoch, val_loaders)
        ''' # gradually unfreeze cnn
        if epoch == opt.epoch_free_cnn:
            for param in model.img_enc.cnn.parameters():
        ...
        rsum = validate(opt, val_loaders[0], model)
        #print ('******* caption val *******')
        #rsum_cap = validate(opt, val_loaders[1], model, '_caption')
        #print ('******* article val *******')
        #rsum_art = validate(opt, val_loaders[2], model, '_article')
        #print ('******* title val *******')
        #rsum_title = validate(opt, val_loaders[3], model, '_title')
        #print ('******* lead val *******')
        #rsum_lead = validate(opt, val_loaders[4], model, '_lead')
...
def train(opt, train_loader, model, epoch, val_loader):
    ...
    if epoch == opt.unfreeze_word_emb:
        model.txt_enc.transformer.src_word_emb_fr.requires_grad = True
        print ('[start finetuning fr word embed!]')
