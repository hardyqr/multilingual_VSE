...
class VDCNN(nn.Module):
    ...
    def forward(self, sentences):
        ...
        #print('after conv, before kmax pooling:', x.shape)
        x = self.kmax_pooling(x)
        #print('after kmax pooling:', x.shape)
        x = x.view(x.size(0), -1)
...
if __name__ == '__main__':
    ...
    args.depth = 9
    args.embed_size = 300
    args.vocabulary_size = 100000
    args.optional_shortcut = True

    model = VDCNN(2,args)
    rand_inputs = Variable(torch.LongTensor(8, 1104).random_(0, 29))
    print(rand_inputs.size())
